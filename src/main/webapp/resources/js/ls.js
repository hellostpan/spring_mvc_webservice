...
function ShowLogoList(page, number) {
    if (CheckLoginStatus() == 1) {
        ...
        ShowLoginBox('ShowLogoList(' + page + ',"' + number + '")');
    }
}
...
//产生20-30随机数
var DTnum = Math.floor(Math.random()*10+20);
var DTs;
//显示倒数秒数
function DTime(isprint, action){
    DTnum -= 1;
    $('#DTime').removeAttr("onclick");
    $('#DTime').text('下载(' + DTnum + ')');
    if(DTnum == 0){
        if (isprint == 1) {
            SaveTemplate(3,'add');
            ...
            SaveTemplate(3,'update');
            return;
        }
    }
    //每秒执行一次,DoTime()
    DTs = setTimeout("DTime('" + isprint + "')",1000);
}

function SaveTemplate(isprint, action) {
    $('#DTime').attr('onclick','DTime();');
    clearTimeout(DTs);
    if (CheckLoginStatus() == 1) {
            ...
            success: function (data) {
                if (data > 0) {
                    if (isprint == 1) {
                        ...
                        var htmlstr = "<div><div><span class='icon-succ02'></span><span style=' color: #7ABD54; font-size: 18px; font-family: \5FAE\8F6F\96C5\9ED1; font-weight: normal; margin-bottom: 15px; '>保存成功，可以下载啦！</span></div><div style='margin-top:20px;'><a href='/Print/Document/" + data + "/?DO=3' class='btn btn-primary'>下载文件</a><div></div>";
                        art.dialog({
                            content: htmlstr,
                            fixed: true,
                            lock: true,
                        ...
                        window.location = "/Document/Design/" + data;
                    } else if (isprint == 0) {
                        var htmlstr = "<div><div><span class='icon-succ02'></span><span style=' color: #7ABD54; font-size: 18px; font-family: \5FAE\8F6F\96C5\9ED1; font-weight: normal; margin-bottom: 15px; '>保存成功，您可以在“会员中心 >> 我的名片模板”查看！</span></div><div style='margin-top:20px;'><a href='/Print/Document/" + data + "' class='btn btn-primary' style='margin-right:10px;'>立即印刷</a><a href='/Document/Design/" + data + "' class='btn btn-primary' style='margin-right:10px;'>继续编辑</a><a href='/Usercenter/MyDocuments' target='_blank' class='btn btn-dake'>我的名片模板</a><div></div>";
                        art.dialog({
                            content: htmlstr,
                            fixed: true,
                            lock: true,
...
function GetText(page, number) {
    ...
    var textValue = $(rObj).val().replace(/[+&]+/,"（如需+&字符,留言客服添加）");
    var textFamily = $(rObj).attr("fn");

    if (/^([A-Za-z]+\s?)*[A-Za-z]$/.test(textFamily)) {
        var reg=/([\u4E00-\u9FA5]|[\uFE30-\uFFA0])+/;
        if(reg.test(textValue)) {
            var textValue = "（英文字体不能含有中文）" + textValue;
        }
    }
